...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const { result } = mutate((s) => {
    const a = s.articles.find((x) => x.type === "case-study" && x.id === id);
    if (!a) return null;
  ...
  if (!result) return NextResponse.json({ error: "Case study not found" }, { status: 404 });
...
export async function DELETE(_req: Request, { params }: Ctx) {
  const id = Number(params.id);
  mutate((s) => {
    s.articles = s.articles.filter((a) => !(a.type === "case-study" && a.id === id));
  });
